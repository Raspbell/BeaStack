...
namespace Global
{
    ...
    {
        // ▼ 1. 自分自身のインスタンスを保持する static 変数
        private static CrossfadeAudioController _instance;
        ...
        // =========================================================
        // ▼ 3. 外部から呼び出すための static メソッド群
        // =========================================================

        /// <summary>
        /// Index指定でBGMをクロスフェード切り替えします。
        /// </summary>
        public static void ChangeClip(int nextIndex)
        {
            if (_instance == null) return;

            if (0 <= nextIndex && nextIndex < _instance.audioClips.Length)
            {
                _instance.BeginCrossFadeToClip(nextIndex);
            }
            else
            {
                _instance.BeginFadeOutCurrent();
            }
        }

        /// <summary>
        /// AudioClipを指定してBGMをクロスフェード切り替えします。
        /// </summary>
        public static void ChangeClip(AudioClip clip)
        {
            if (_instance == null) return;

            if (clip != null)
            {
                _instance.BeginCrossFadeToClip(clip);
            }
            else
            {
                _instance.BeginFadeOutCurrent();
